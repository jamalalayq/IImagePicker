...
public final class IImagePicker: NSObject {
    
    private var picker: UIImagePickerController?
    private var tune: Tuning!
    public var onPickImage: ((IResource, IImagePicker)->())?,
    onCancel: (()->())?,
    onError: (()->())?,
    onPickVideoURL: ((URL, IImagePicker)->())?
...
private extension IImagePicker {
    ...
    func openPhotoLibrary() {
        ...
        guard let picker = picker else {
            print(#function, "use `onError` block because IImagePicker is null")
            onError?()
            onError = .none
            return
        }
...
extension IImagePicker: IImagePickerDelegate  {
    ...
    public func imagePickerController(_ picker: UIImagePickerController,
        ...
        if let videoURL = info[.mediaURL] as? URL {
            ...
            let resource = IResource(name: name, extension: ext, image: pickedImage, url: url)
